...
def harr_mutilevel_recomp(*parm):
...
# def harr_2D_recomposition(*coeffs, level):
...
# def harr_2D_decomposition(*coeffs, level):
...
def harr_mutilevel_decomp(data, model='symmetric', level=1):
    low = np.asarray(LOW_DECOMP, dtype=np.float32)
    hiht = np.asarray(HIGT_DECOMP, dtype=np.float32)
...
def harr_decomposition(data, model='symmetric', level=1):
    ...
    if data.ndim == 1:
        if level == 1:
            cA, cD = harr_single_decomp(x, model=model)
            return cA, cD
        else:
            coeffs = harr_mutilevel_decomp(data, model=model, level=level)
        return coeffs


def harr_recomposition(*parm):
    if len(parm) == 2:
        cA, cD = parm
        data = harr_single_decomp(cA, cD)
        return data
        ...
        if len(coeffs) >= 3:
            if isinstance(coeffs[-1], np.ndarray):
                data = harr_mutilevel_recomp(coeffs)
                return data
